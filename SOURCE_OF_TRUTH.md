...
- Ranking status: RANKED (all accounts active), PROVISIONAL (stale heartbeat with open trades), UNRANKED (tracking lost with open trades)
- **Ranking enforcement**: UNRANKED traders are excluded from leaderboard entries entirely. PROVISIONAL traders remain on leaderboard with visible badge. `calculateRankings()` calls `checkRankingEligibility()` per trader.
...
- **Public API**: statement route (`/api/users/[userId]/trader-statement`) is public — no auth required. Strips raw equity drawdown from response. Only NAV-based (cash-flow-neutral) drawdown exposed publicly.
...
| 2026-03-20 | Single-statement architecture: close 3 gaps | (1) Statement API now public — no auth required, strips raw equity drawdown (only NAV-based exposed). MT account details remain auth-gated on profile page. (2) Tracking thresholds: E2E docs updated from stale 60/120s to repo-truth 180/300s. (3) Ranking status enforced end-to-end: `calculateRankings()` calls `checkRankingEligibility()`, UNRANKED traders excluded from leaderboard, PROVISIONAL shown with badge. Statement page shows ranking status badge. Regression tests added. | trader-statement/route.ts, ranking.service.ts, LeaderboardTable.tsx, TraderStatementView.tsx, trader-statement.ts, profile/[userId]/page.tsx, E2E-INTEGRITY-TEST.md, CLAUDE.md, SOURCE_OF_TRUTH.md |
